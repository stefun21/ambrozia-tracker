...
function buildData(lat: number, lon: number, weather: WeatherPayload | null, air: AirPayload | null): AppData {
  ...
  const detail = source === "Mixt" ? "Date live + model meteo" : "Estimare meteo/sezon";
...
function App() {
  ...
  return (
    <main className="screen" style={{ ["--accent" as string]: theme.color, ["--glow" as string]: theme.glow, ["--soft" as string]: theme.soft }}>
      <style>{styles}</style>
      <section className="phone-card">
        <header className="topbar">
          ...
          <button className="refresh" type="button" onClick={() => startLocationFlow(true)} aria-label="Actualizează datele">
            {isRefreshing ? "⟳" : "↻"}
          </button>
        </header>
        ...
        <section className="hero">
          <div className="score-ring">
            <svg className="ring" viewBox="0 0 200 200" aria-hidden="true">
              <circle className="ring-track" cx="100" cy="100" r="87" />
              <circle className="ring-progress" cx="100" cy="100" r="87" strokeDasharray={`${dash} ${circumference - dash}`} />
            </svg>
        ...
        <section className="mini-grid">
          <Metric label="Acum" value={`${weatherIcon(data.weatherCode)} ${data.tempNow === null ? "--" : `${Math.round(data.tempNow)}°`}`} />
          <Metric label="Max" value={data.tempMax === null ? "--" : `${Math.round(data.tempMax)}°`} />
          <Metric label="Vânt" value={data.wind === null ? "--" : `${Math.round(data.wind)} km/h`} />
        </section>

        <section className="forecast-row" aria-label="Prognoză ambrozie">
  ...
  * { box-sizing: border-box; }
  ...
  .screen {
    ...
    background:
      ...
      radial-gradient(circle at 85% 18%, rgba(168, 85, 247, 0.16), transparent 34%),
      linear-gradient(145deg, #050816 0%, #0f172a 48%, #111827 100%);
    overflow: hidden;
  }
  .phone-card {
    position: relative;
    ...
    box-shadow: 0 30px 90px rgba(0,0,0,0.46), inset 0 1px 0 rgba(255,255,255,0.12);
  ...
  .phone-card::before {
    ...
    pointer-events: none;
    background: radial-gradient(circle at 50% 27%, var(--glow), transparent 36%);
    opacity: 0.9;
  }
  ...
  .refresh {
    width: 44px; height: 44px; border: 0; border-radius: 18px;
  ...
  .status-pill {
    ...
    font-size: 12px; font-weight: 950; margin-bottom: 12px;
  }
  ...
  .metric, .forecast-card, .bottom-card {
  ...
  .loading-card {
    width: min(100%, 390px); min-height: 460px; border-radius: 36px; padding: 36px 28px;
  ...
  .loading-orb {
    ...
    background: radial-gradient(circle at 30% 25%, #ffffff, #2dd4bf 38%, #0f766e 80%);
    color: #07111f; font-size: 34px; margin-bottom: 24px;
    box-shadow: 0 0 48px rgba(45,212,191,0.35);
  ...
  @media (max-width: 390px) {
    .phone-card { height: calc(100svh - 18px); padding: 14px; border-radius: 28px; gap: 9px; }
    .city { font-size: 16px; max-width: 230px; }
    .refresh { width: 40px; height: 40px; border-radius: 16px; }
    .hero { grid-template-columns: 178px 1fr; min-height: 194px; gap: 8px; }
    .score-ring { width: 178px; height: 178px; }
    .score-content { width: 132px; height: 132px; }
    .score-number { font-size: 50px; }
    .hero-copy h1 { font-size: 25px; }
    .hero-copy p { font-size: 12px; }
    .metric { padding: 10px 6px; border-radius: 17px; }
    .metric strong { font-size: 17px; }
    .forecast-card { padding: 8px 4px; border-radius: 15px; }
    .forecast-card strong { font-size: 19px; }
    .bottom-card { padding: 11px; border-radius: 18px; }
    .notice { font-size: 11px; }
  }
  @media (max-height: 700px) {
    .phone-card { height: calc(100svh - 12px); padding: 12px; gap: 7px; border-radius: 24px; }
    .eyebrow { display: none; }
    .city { font-size: 15px; }
    .hero { grid-template-columns: 150px 1fr; min-height: 162px; }
    .score-ring { width: 150px; height: 150px; }
    .score-content { width: 112px; height: 112px; }
    .score-label { font-size: 9px; }
    .score-number { font-size: 42px; }
    .score-scale { font-size: 12px; }
    .hero-copy h1 { font-size: 22px; }
    .hero-copy p { margin-top: 6px; font-size: 11px; }
    .status-pill { margin-bottom: 7px; padding: 5px 8px; font-size: 10px; }
    .metric { padding: 8px 5px; }
    .metric span, .forecast-card span { font-size: 10px; }
    .metric strong { font-size: 16px; }
    .forecast-card { padding: 7px 4px; }
    .forecast-card strong { margin-top: 3px; font-size: 17px; }
    .bottom-card { padding: 9px 10px; }
    .bottom-card strong { font-size: 12px; }
    .bottom-card span, .live-dots span, .notice { font-size: 10px; }
  }
`;
