...
function App() {
  ...
  const fetchData = async (lat: number, lon: number, namePrefix = "") => {
    try {
      ...
      const cityName =
        namePrefix + (cityJson.city || cityJson.locality || cityJson.principalSubdivision || "Oraș");
      ...
      let hourIndex = hourlyTimes.findIndex((time) => time.startsWith(nowIso));
      ...
      const nowVal =
        getSafeNumber(birch[hourIndex]) +
        getSafeNumber(grass[hourIndex]) +
        getSafeNumber(ragweed[hourIndex]);

      const nextVal =
        getSafeNumber(birch[hourIndex + 1]) +
        getSafeNumber(grass[hourIndex + 1]) +
        getSafeNumber(ragweed[hourIndex + 1]);

      const score = Math.min(Math.max(nowVal / 15, 0), 10);

      const trend: "↑" | "↓" | "→" =
        nextVal > nowVal ? "↑" : nextVal < nowVal ? "↓" : "→";

      const payload: AppData = {
        score,
        trend,
        tempNow: Math.round(getSafeNumber(weatherJson.current?.temperature_2m)),
        tempMax: Math.round(getSafeNumber(weatherJson.daily?.temperature_2m_max?.[0])),
        tempMin: Math.round(getSafeNumber(weatherJson.daily?.temperature_2m_min?.[0])),
        advice:
          score > 7
            ? "🛑 Geamuri închise!"
            : score > 3
        ...
        forecast: (weatherJson.daily?.time ?? []).slice(1, 7).map((time: string, index: number) => ({
          ...
          temp: Math.round(getSafeNumber(weatherJson.daily?.temperature_2m_max?.[index + 1])),
          icon: getWeatherIcon(getSafeNumber(weatherJson.daily?.weather_code?.[index + 1])),
        })),
      };
      ...
      console.error(err);
      setError("Nu am putut încărca datele. Folosesc București ca fallback.");
      ...
      <div className="container">
        ...
        >
          {data.forecast.map((item) => (
            <div key={item.day}>
              <div style={{ fontSize: "0.7rem", fontWeight: 800, opacity: 0.65 }}>
                {item.day}
              </div>
        ...
        {error && (
          <div style={{ marginTop: 16, fontSize: "0.8rem", opacity: 0.7 }}>
            {error}
          </div>
        )}
      </div>
