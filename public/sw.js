...
function App() {
  ...
  useEffect(() => {
    ...
    const fallbackTimer = window.setTimeout(updateInstallVisibility, 1400);
  ...
  return (
    <main className="app-shell">
      ...
      <section className="phone-card" style={{ ["--accent" as string]: theme.color, ["--glow" as string]: theme.glow, ["--soft" as string]: theme.soft }}>
        ...
        <header className="topbar">
          ...
          <div className="top-actions">
            {showInstall && !isInstalled && (
              <button className="install-compact" onClick={installApp} aria-label="Instaleaza aplicatia pe ecranul principal">
                <span>＋</span> App
              </button>
            )}
        ...
        {installHelpOpen && (
          <div className="install-sheet-backdrop" role="presentation" onClick={() => setInstallHelpOpen(false)}>
            <div className="install-sheet" role="dialog" aria-modal="true" aria-label="Instaleaza aplicatia" onClick={(event) => event.stopPropagation()}>
              <button className="install-sheet-close" onClick={() => setInstallHelpOpen(false)} aria-label="Inchide">×</button>
              <div className="install-sheet-icon">＋</div>
              <h3>Adauga pe ecran</h3>
              <p>Pe Android, deschide meniul Chrome <strong>⋮</strong> si alege <strong>Instaleaza aplicatia</strong> sau <strong>Adauga pe ecranul de pornire</strong>.</p>
              <small>Daca vezi inca iconita veche, sterge shortcut-ul vechi si intra din nou pe site dupa ultimul deploy.</small>
            </div>
          </div>
        )}
...
const styles = `
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .topbar, .hero-card, .forecast-grid, .metrics-grid, .status-card, .install-compact, .install-sheet-backdrop, .update-banner { position: relative; z-index: 2; }
  ...
  .install-compact {
    ...
    padding: 0 13px;
    color: #eaffc1;
    background: linear-gradient(135deg, rgba(163, 230, 53, 0.18), rgba(34, 197, 94, 0.12));
    box-shadow: 0 12px 32px rgba(34, 197, 94, 0.18), inset 0 1px 0 rgba(255, 255, 255, 0.08);
    font-size: 13px;
    font-weight: 850;
  }
  .install-compact span { width: 20px; height: 20px; display: grid; place-items: center; border-radius: 999px; color: #04111a; background: #a3e635; font-size: 14px; line-height: 1; }
  .refresh {
    width: 48px;
    height: 48px;
    display: grid;
  ...
  .install-sheet {
    ...
    border-radius: 28px;
    padding: 22px;
    background: linear-gradient(160deg, rgba(10, 25, 24, 0.98), rgba(3, 7, 18, 0.99));
  ...
  .install-sheet h3 { margin: 14px 0 8px; font-size: 24px; letter-spacing: -0.04em; }
  .install-sheet p { margin: 0; color: rgba(226, 232, 240, 0.82); line-height: 1.55; }
  .install-sheet small { display: block; margin-top: 12px; color: rgba(226, 232, 240, 0.56); line-height: 1.45; }
  ...
  @media (max-width: 380px) {
    ...
    h1 { font-size: 25px; }
    h2 { font-size: 27px; }
    .install-compact { width: 48px; padding: 0; justify-content: center; font-size: 0; }
  }
`;
